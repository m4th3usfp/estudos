...
document.addEventListener('DOMContentLoaded', function() {
    ...
    container.innerHTML = gerarHTML(pessoasSalvas);

    // Adicionar evento blur ao campo de entrada de URL
    const urlInput = document.getElementById('URL');
    urlInput.addEventListener('blur', function() {
        const urlValue = urlInput.value;
        atualizarTabela(urlValue);
    });
});

function atualizarTabela(url) {
    // Verificar se a URL inserida corresponde a alguma das URLs já salvas
    const pessoaEncontrada = pessoasSalvas.find(pessoa => pessoa.URL === url);
    if (pessoaEncontrada) {
        // Se uma correspondência for encontrada, exiba os dados correspondentes à URL encontrada na tabela
        const dados = [{ nome: pessoaEncontrada.nome, email: pessoaEncontrada.email, tipo: pessoaEncontrada.tipo }];
        atualizarTabelaComDados(dados);
    } else {
        console.log('Não foram encontrados dados para a URL inserida:', url);
    }
}
    ...
    dados.forEach(pessoa => {
        ...
        newRow.innerHTML = `
            ...
            <td>
                ...
                <button class="btn btn-danger" type="button" onclick="cancelarLinha(this)">Cancelar</button>
            </td>
        `;
...
function cancelarLinha(botao) {
    ...
    const h5 = document.getElementById('h5'); // Corrigido para obter o elemento correto

    // Remove a linha
    linha.remove();

    // Se o número de linhas no tbody for igual a 1, exibe a mensagem "Nenhuma pessoa cadastrada"
    if (tbody.querySelectorAll('tr').length === 0) {
